...
def popular_products(request):
    ...
    popular_products = CartItem.objects.exclude(cart=cart.id).annotate(c=Count('product'))[:3]
    return render_to_response('personalised_catalog.html', {'popular_products':popular_products})
    
def get_total_product_price(request, product_id):
    if request.is_ajax():
        price =  Product.objects.filter(id=product_id).values('price')
    return HttpResponse('')  
